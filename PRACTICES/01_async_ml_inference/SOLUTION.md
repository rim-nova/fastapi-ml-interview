...
This solution implements asynchronous ML inference using FastAPI's `BackgroundTasks`. The key insight is that users
shouldn't wait for slow ML operations - instead, they get a job ID immediately and can poll for results.
